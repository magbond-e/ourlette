...
import { Bell, AlertTriangle, Clock, Sparkles, PartyPopper, CheckCheck, Trash2, ChevronRight, X } from 'lucide-react';
...
export const NotificationDropdown: React.FC = () => {
  ...
  const filteredNotifications = notifications.filter((item) => {
    ...
    if (filter === 'alerts') return item.type === 'order_overdue' || item.type === 'order_due_soon';
    return true;
  });
  ...
  const getItemBadgeBg = (type: NotificationType, read: boolean) => {
    if (read) return 'bg-sable/20 border-sable/40';
    switch (type) {
      case 'order_overdue':
      ...
      case 'feature_update':
        return 'bg-amber-500/10 border-amber-500/20 text-sombre';
      default:
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      {/* Popover Dropdown */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-[340px] sm:w-[400px] max-w-[calc(100vw-32px)] bg-white border border-sable/60 rounded-3xl shadow-2xl z-50 overflow-hidden font-sans animate-in fade-in zoom-in-95 duration-150">
          {/* Header */}
          ...
          {/* Filter Tabs */}
          <div className="px-3 pt-2.5 pb-2 bg-white border-b border-sable/40 flex items-center gap-1.5 overflow-x-auto text-xs">
            <button
              onClick={() => setFilter('all')}
              className={`px-3 py-1 rounded-full font-bold transition-all ${filter === 'all'
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full font-bold transition-all ${filter === 'unread'
                  ? 'bg-sombre text-white shadow-xs'
                  : 'bg-[#F5F5F3] text-sombre/70 hover:bg-sable/40'
                }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full font-bold transition-all ${filter === 'alerts'
                  ? 'bg-sombre text-white shadow-xs'
                  : 'bg-[#F5F5F3] text-sombre/70 hover:bg-sable/40'
                }`}
            >
          ...
          <div className="max-h-[360px] overflow-y-auto divide-y divide-sable/30">
            {filteredNotifications.length === 0 ? (
              <div className="p-8 text-center space-y-2">
                ...
                <p className="text-[11px] text-sombre/50">Vous serez averti en cas de retards ou rappels de livraison.</p>
              </div>
            ) : (
              filteredNotifications.map((item) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3.5 flex items-start gap-3 transition-colors cursor-pointer group ${item.read
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 space-y-0.5">
                      <div className="flex items-center justify-between gap-1">
                        <h4 className={`text-xs font-bold truncate ${item.read ? 'text-sombre/80' : 'text-sombre font-extrabold'}`}>
                          {item.title}
                        </h4>
                        {!item.read && (
                    ...
                    {/* Arrow hint if link */}
                    {item.link && (
                      <ChevronRight className="w-4 h-4 text-sombre/30 group-hover:text-accent group-hover:translate-x-0.5 transition-all self-center shrink-0" />
                    )}
                  </div>
                );
              })
